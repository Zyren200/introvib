...
const MatchCard = ({ match, onSendHello, onSaveLater }) => {
  ...
  return (
    <div className="rounded-[1.6rem] border border-border bg-background/80 p-4 shadow-gentle-sm transition-gentle hover:border-primary/30 hover:shadow-gentle">
      <div className="mb-4 flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            {match?.isOnline && (
              <span className="caption rounded-full bg-success/10 px-2.5 py-1 text-success">
                Active now
              </span>
            )}
            {match?.status === 'messaged' && (
              <span className="caption rounded-full bg-accent/10 px-2.5 py-1 text-accent">
                Messaged
              </span>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {match?.sharedInterests
            ...
            ?.map((interest, index) => (
              <span
                key={index}
                className="caption rounded-full bg-accent/10 px-3 py-1.5 font-medium text-accent"
              >
                {interest}
              </span>
            ))}
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {match?.personalityTags?.map((tag, index) => (
            <span
              key={index}
              className="caption rounded-full bg-secondary/10 px-3 py-1.5 text-secondary-foreground"
            >
              {tag}
            </span>
          ))}
